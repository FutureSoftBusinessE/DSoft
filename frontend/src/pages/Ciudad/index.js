...
const Ciudad = () => {
  ...
  const { mutateAsync: SaveEliminacionCiudad, isPending: isDeletingCiudad } = useMutation({
    queryKey: ["isDeletingCiudad"],
    fn: async (data) => {
      const response = await api.post("/Ciudad/eliminarCiudad", data)
      return response.data
    },
    showError: "modal",
    showSuccess: "toast",
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: ["Ciudad"] })
    },
  })
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <div className="main main-app p-3 p-lg-4">
        ...
        <CustomBackdrop isLoading={isDeletingCiudad} />

        <Box className={StyledRoot}>
          <ModalImportCSV
            open={openModal}
            onClose={() => setOpenModal(false)}
            templateFileName="PlantillaCreacionCiudad.csv"
            fieldConfigs={{
              ciucodigo: { required: true, key: true },
              ciudescri: { required: true },
              ciustatus: { required: false },
              ciufecsys: { required: false },
              ciuhorsys: { required: false },
              ciuususys: { required: false },
              ciudinardap: { required: false },
            }}
            maxFileSize={10 * 1024 * 1024}
            validateEndpoint="/Ciudad/validarCiudadIMP"
            insertEndpoint="/Ciudad/insertarCiudadIMP"
            onImportComplete={() => qc.invalidateQueries({ queryKey: ["Ciudad"] })}
          />

          <CustomConditionalActionsTableServer
            endpoint="/Ciudad/getAllCiudad"
            errorMsgFilterSearch="Error en cargar datos"
            queryKeyModal="Ciudad"
            perPage={10}
            ...
            rowActions={(row) => {
              ...
              const actions = [
                ...
                },
                {
                  ...
                  onClick: async (r) => {
                    try {
                      await SaveEliminacionCiudad(r.original)
                    } catch (err) {
                      console.error("Error eliminando la ciudad:", err)
                    }
                  },
                },
              ]
                ...
                },
                {
                  ...
                  actions: [
                      ...
                      onClick: ({ columns, data }) => {
                        if (device === "sm") {
                          return handleExportDataPdfSMScreen(
                            ...
                            data,
                            "Reporte de Ciudades",
                            `Reporte de Ciudades ${new Date().toLocaleString()}`,
                          )
                        }
                        handleExportDataPdfLGScreen(
                          columns,
                    ...
                    },
                    {
                      ...
                      icon: getIconComponent(exportarAction?.accnameicono, exportarAction?.acctipoico),
                      onClick: ({ data }) => {
                        handleAllExportDataCSV(data, `Reporte de Ciudades ${new Date().toLocaleString()}`)
                      },
                    },
                  ],
                },
            ...
            columnsTable={[
              {
                accessorKey: "ciucodigo",
                header: "Código",
                size: 130,
                Cell: ({ cell }) => <span>{cell.getValue()}</span>,
              },
              {
                accessorKey: "ciudescri",
                header: "Descripción",
                size: 280,
                Cell: ({ cell }) => <span>{cell.getValue()}</span>,
              },
              {
                accessorKey: "ciustatus",
                header: "Estado",
                size: 120,
                Cell: ({ cell }) => <span>{cell.getValue()}</span>,
              },
              {
                accessorKey: "ciufecsys",
              ...
              {
                accessorKey: "ciuususys",
                header: "Creado Por",
                size: 180,
                Cell: ({ cell }) => <span>{cell.getValue()}</span>,
              },
              {
                accessorKey: "ciudinardap",
                header: "DINARDAP",
                size: 140,
                Cell: ({ cell }) => <span>{cell.getValue()}</span>,
              },
