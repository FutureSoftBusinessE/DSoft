...
# Helper para validar Cargos (Clonado de tu estándar SIAC)
def validar_cargo(connection, columns: list, required: list, key_columns: list, rows: list):
    ...
    for i, fila in enumerate(rows):
        ...
        # Duplicados en el mismo archivo CSV
        clave = []
    ...
    # Buscar existentes en Base de Datos
    cols_sql = ", ".join(key_columns)
    cia_val = rows[0]["ciacodigo"]

    # Query específico para Cargos
    sql_get_all = text(f"SELECT {cols_sql} FROM rhbcargos WHERE ciacodigo = :ciacodigo")
    ...
    for fila in rows:
        ...
        if tuple(clave_fila) in existentes:
            fila["ok"] = False
            fila["feedback"] = "Código de Cargo ya existe en el sistema"
...
@bp.route("/validarCargosIMP", methods=["POST"])
@jwt_required()
@api_endpoint
def validarCargosIMP():
    claims = get_jwt()
