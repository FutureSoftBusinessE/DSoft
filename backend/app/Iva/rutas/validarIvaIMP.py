...
def validar_iva(connection, columns: list, required: list, key_columns: list, rows: list):
    ...
    # Indagando a ivafecini (diccionario) en la primera fila
    iva_val = rows[0]["ivafecini"]
    sql_get_all = text(f"SELECT {cols_sql} FROM siaciva WHERE ivafecini = :ivafecini")
    rows_db = connection.execute(sql_get_all, {"ivafecini": iva_val}).mappings().all()
...
@bp.route("/validarIvaIMP", methods=["POST"])
@jwt_required()
@api_endpoint
def validarIvaIMP():
    claims = get_jwt()
