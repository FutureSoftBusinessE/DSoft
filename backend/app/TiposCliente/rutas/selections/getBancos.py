...
@bp.route("/getBancos", methods=["POST"])
@cross_origin()
@jwt_required()
def getBancos():
    claims = get_jwt()
    ...
    try:
        with engine.connect() as connection:
            if typeSearch == SEARCH_TYPE_HELPER.ID_SEARCH:
                # Search by code: bcocodigo
                bcocodigo = data.get("bcocodigo", "").strip()
                query = text(
                    """
                    SELECT ciacodigo, bcocodigo, bcodescri
                    FROM cxcbbco
                    WHERE ciacodigo = :ciacodigo AND bcocodigo = :bcocodigo
                    """
                )
                row = connection.execute(query, {"ciacodigo": sCodCia, "bcocodigo": bcocodigo}).mappings().fetchone()

                if row:
                    return jsonify({"data": dict(row)})
                else:
                    raise ValidationError(f"Banco no encontrado: {bcocodigo}")
        ...
        raise ValidationError(f"Error obteniendo bancos: {str(e)}")
