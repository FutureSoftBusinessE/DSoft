...
const LargeScreenTable = ({
  ...
  const table = useMaterialReactTable({
    ...
    // MODIFICADO: Se habilita la selección de múltiples filas
  ...
  // MODIFICADO: Se actualiza el efecto para enviar todas las filas seleccionadas
  useEffect(() => {
    const selectedRows = table.getSelectedRowModel().rows
    if (selectedRows.length > 0) {
      onItemSelected(selectedRows.map((row) => row.original))
    } else {
      onItemSelected([]) // Enviar arreglo vacío si no hay selección
    }
...
const SmallScreenTable = ({
  ...
  // Estado para almacenar las filas seleccionadas (selección múltiple)
  const [selectedRows, setSelectedRows] = useState([])

  // Efecto para notificar los registros seleccionados si se pasa la función onItemSelected
  useEffect(() => {
    if (onItemSelected) {
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
          {isLoading ? (
            ...
            data.map((row, rowIndex) => (
              ...
              >
                {/* Contenedor para checkbox y acciones: se ubica en la esquina superior derecha */}
                <Box
                  sx={{
                    position: "absolute",
                    top: 0,
                ...
                >
                  <Checkbox
                    ...
                    onClick={(e) => {
                      e.stopPropagation()
                      // Si ya está seleccionado, lo deselecciona; de lo contrario, lo agrega
...
// --------------------------------------------------
// --------------------------------------------------
//            CustomMultiRowSelectionServerSide
// --------------------------------------------------
// --------------------------------------------------

const CustomMultiRowSelectionServerSide = ({
  ...
  onHandleSelectedData, // Esta es una funcion que se usara cuando se haya seleccionada alguna fila de la tabla
}) => {
  ...
  // MODIFICADO: Se inicializa itemSelected como arreglo para múltiples selecciones
  const [itemSelected, setItemSelected] = useState([]) // Selected items in table
  ...
  //   // MODIFICADO: Se actualiza el efecto para verificar selección múltiple usando la longitud del arreglo
  //   useEffect(() => {
  ...
  return (
    <Box>
      ...
      <div style={{ textAlign: "center", marginBottom: "15px" }}>
        ...
        >
          Agregar todos los servicios seleccionados
        </Button>
      </div>
      <CustomTable
        data={data || []}
...
export default CustomMultiRowSelectionServerSide
