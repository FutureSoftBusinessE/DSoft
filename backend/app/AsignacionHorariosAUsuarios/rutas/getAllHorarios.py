...
@bp.route("/getAllHorarios", methods=["POST"])
@cross_origin()
@jwt_required()
def getAllHorarios():
    claims = get_jwt()
    ...
    with engine.connect() as connection:
        with connection.begin():
            ...
            allowed_columns = [{"usrcodigo": FILTER_VALUE_TYPE.STRING}, {"locdescri": FILTER_VALUE_TYPE.STRING}]
            ...
            base_query = """
                SELECT
                    ...
                    locdescri,
                    usrnombre,
                    MAX(hrfecmsys) AS hrfecmsys,
                    MAX(hrhormsys) AS hrhormsys
                FROM rhbhorarios
                GROUP BY usrcodigo, loccodigo, locdescri,usrnombre
            """
