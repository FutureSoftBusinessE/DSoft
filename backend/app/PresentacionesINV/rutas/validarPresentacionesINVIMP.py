...
# Helper para validar la integridad de las presentaciones antes de la importación
def validar_presentacionesinv(connection, columns: list, required: list, key_columns: list, rows: list):
    ...
    # Validar que las columnas clave estén configuradas correctamente
    ...
    for i, fila in enumerate(rows):
        ...
        # Verificar campos obligatorios vacíos (precodigo, predescri)
        faltantes = []
        ...
        # Detectar duplicados dentro del mismo archivo basándose en precodigo
        clave = []
        ...
        if clave in vistos:
            fila["ok"] = False
            fila["feedback"] = f"Registro duplicado en el archivo (Presentación {clave[0]})"
            continue

        vistos.add(clave)

    # 3. Verificación contra Base de Datos (Multitenancy - inbpre)
    cols_sql = ", ".join(key_columns)
    cia_val = rows[0]["ciacodigo"]
    
    # Consultamos las presentaciones ya registradas para la compañía actual
    sql_get_all = text(f"SELECT {cols_sql} FROM inbpre WHERE ciacodigo = :ciacodigo")
    ...
    # Marcar registros que ya existen en la tabla inbpre
    for fila in rows:
        if not fila["ok"]:
            continue
        ...
        if tuple(clave_fila) in existentes:
            fila["ok"] = False
            fila["feedback"] = "Esta Presentación ya existe en la base de datos"
...
@bp.route("/validarPresentacionesINVIMP", methods=["POST"])
@cross_origin()
@jwt_required()
@api_endpoint
def validarPresentacionesINVIMP():
