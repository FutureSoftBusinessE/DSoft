...
const CrearProvincia = () => {
  ...
  const {
    mutateAsync: SaveCreacionProvincia,
    isPending: isSavingCreacionProvincia,
    isError: isErrorSavingCreacion,
  } = useMutation({
    queryKey: ["isCreatingProvincia"],
    fn: async (data) => {
      const response = await api.post("/Provincia/crearProvincia", data)
  ...
  const { mutateAsync: GetSiguienteCodigoProvincia } = useMutation({
    queryKey: ["getSiguienteCodigoProvincia"],
    fn: async () => {
      const response = await api.post("/Provincia/getSiguienteCodigoProvincia", {})
      return response
    },
    showError: "modal",
    showSuccess: false,
  ...
  useEffect(() => {
    const loadCode = async () => {
      try {
        const result = await GetSiguienteCodigoProvincia()
        setFormData((prev) => ({
          ...prev,
          procodigo: result.data?.procodigo || "",
        }))
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    const provinciaDataToSave = {
      ...formData,
    }

    try {
      if (!provinciaDataToSave.prodescri) {
        showWarning("Ingresa la descripción de la Provincia")
        return
      }

      await SaveCreacionProvincia(provinciaDataToSave)
    } catch (error) {
    ...
    <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={esLocale}>
      <ThemeProvider theme={theme}>
        ...
        <div className="main main-app p-3 p-lg-4">
            ...
            {toolbarActions.map((action) => (
              <Tooltip title={action.label} key={action.key}>
                <IconButton
                  ...
                  disabled={isSavingCreacionProvincia || isLoadingCode}
                  sx={{
                    border: "1px solid",
                    borderColor: "divider",
                    borderRadius: 1,
          ...
          >
            <b>Crear Nueva Provincia</b>
          </div>

          <CustomBackdrop isLoading={isSavingCreacionProvincia || isLoadingCode} />
