...
# Helper para validar la integridad de los vendedores antes de la importación
def validar_vendedoresdf(connection, columns: list, required: list, key_columns: list, rows: list):
    ...
    for i, fila in enumerate(rows):
        ...
        # Verificar campos obligatorios vacíos basados en la lista requerida (vencodigo, vennombre)
        faltantes = []
        ...
        # Detectar duplicados dentro del mismo archivo basándose en vencodigo
        clave = []
    ...
    # 3. Verificación contra Base de Datos (Multitenancy - fapvendedor)
    cols_sql = ", ".join(key_columns)
    cia_val = rows[0]["ciacodigo"]
    
    # Consultamos los códigos de vendedores ya registrados para la compañía actual
    sql_get_all = text(f"SELECT {cols_sql} FROM fapvendedor WHERE ciacodigo = :ciacodigo")
    ...
    # Marcar registros que ya existen en la tabla fapvendedor
    for fila in rows:
        if not fila["ok"]:
            continue
        ...
        if tuple(clave_fila) in existentes:
            fila["ok"] = False
            fila["feedback"] = "Este código de Vendedor ya existe en la base de datos"
...
@bp.route("/validarVendedoresDFIMP", methods=["POST"])
@cross_origin()
@jwt_required()
@api_endpoint
def validarVendedoresDFIMP():
