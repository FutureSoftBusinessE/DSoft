...
const CrearContraCliDF = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    clicodigo: "", clinombre: "", concodigo: "", condescri: "",
  ...
  const listaClientes = (Array.isArray(rawInitialData?.clientes) ? rawInitialData.clientes : []).map(i => ({ 
    id: i.clicodigo, label: `${i.clicodigo} - ${i.clinombre || ""}`, nombre: i.clinombre 
  }))
  const listaTipos = (Array.isArray(rawInitialData?.tiposContrato) ? rawInitialData.tiposContrato : []).map(i => ({ 
    id: i.concodigo, label: `${i.concodigo} - ${i.condescri || ""}`, frecuencia: i.confrecuencia, descri: i.condescri 
  }))
  const listaArticulos = (Array.isArray(rawInitialData?.articulos) ? rawInitialData.articulos : []).map(i => ({ 
    id: i.artcodigo, label: `${i.artcodigo} - ${i.artdescri || ""}`, invcodigo: i.invcodigo, artdescri: i.artdescri, precio1: i.precio1 
  }))
  ...
  const generarPeriodos = () => {
    ...
    const start = new Date(formData.confecinifac + "T00:00:00"); const end = new Date(formData.confecfin + "T00:00:00")
    if (start > end) return showWarning("Fecha de inicio no puede ser mayor a la final.")
    let current = new Date(start); const newPeriodos = []
    while (current <= end) {
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <div className="main main-app p-3 p-lg-4">
        ...
        <div style={{ display: "flex", justifyContent: "center", mb: "30px", fontSize: "25px" }}><b>Creación de Contrato de Cliente</b></div>
        ...
        <Box sx={StyledRoot} component="form" onSubmit={handleSubmit}>
          ...
          <Accordion defaultExpanded sx={accordionStyles}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}><AssignmentIcon color="primary" fontSize="small" /><Typography variant="subtitle2" fontWeight="bold" color="primary">DATOS GENERALES</Typography></Box>
            </AccordionSummary>
            <AccordionDetails>
              <Grid container spacing={3}>
                <Grid item xs={12} sm={3}><TextField disabled fullWidth label="Nº Contrato" value={formData.concodcontrato} sx={{ bgcolor: "#f0f0f0" }} /></Grid>
                ...
                <Grid item xs={12} sm={9}><TextField fullWidth label="Descripción" value={formData.condescri} onChange={(e) => handleInputChange("condescri", e.target.value)} /></Grid>
                <Grid item xs={12} sm={3}><TextField select fullWidth label="Estado" value={formData.constatus} onChange={(e) => handleInputChange("constatus", e.target.value)}><MenuItem value="A">ACTIVO</MenuItem><MenuItem value="I">INACTIVO</MenuItem></TextField></Grid>
                <Grid item xs={12} sm={3}><TextField fullWidth type="date" label="F. Inicio" InputLabelProps={{ shrink: true }} value={formData.confecinicio} onChange={(e) => handleInputChange("confecinicio", e.target.value)} /></Grid>
                <Grid item xs={12} sm={3}><TextField fullWidth type="date" label="F. Fin" InputLabelProps={{ shrink: true }} value={formData.confecfin} onChange={(e) => handleInputChange("confecfin", e.target.value)} /></Grid>
                <Grid item xs={12} sm={3}><TextField fullWidth type="date" label="F. Firma" InputLabelProps={{ shrink: true }} value={formData.confecfirma} onChange={(e) => handleInputChange("confecfirma", e.target.value)} /></Grid>
                <Grid item xs={12} sm={3}><TextField fullWidth type="date" label="F. Facturación" InputLabelProps={{ shrink: true }} value={formData.confecinifac} onChange={(e) => handleInputChange("confecinifac", e.target.value)} /></Grid>
                <Grid item xs={12} sm={6}><TextField fullWidth label="Frecuencia" value={formData.confrecuencia} InputProps={{ readOnly: true }} sx={{ bgcolor: "#f9f9f9" }} /></Grid>
                <Grid item xs={12} sm={6}><TextField fullWidth label="Total ($)" value={formData.convalor} InputProps={{ readOnly: true }} sx={{ bgcolor: "#f0f0f0" }} /></Grid>
              </Grid>
            </AccordionDetails>
          </Accordion>
          ...
          <Accordion defaultExpanded sx={accordionStyles}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}><ListAltIcon color="primary" fontSize="small" /><Typography variant="subtitle2" fontWeight="bold" color="primary">DETALLE DE SERVICIOS</Typography></Box>
            </AccordionSummary>
            <AccordionDetails>
              <Box sx={{ mb: 2, display: "flex", justifyContent: "flex-end" }}><Button variant="outlined" startIcon={<AddCircleIcon />} onClick={() => setServicios([...servicios, { invcodigo: "01", artcodigo: "", artdescri: "", concantidad: 1, convalor: "0.00", contotal: "0.00" }])}>Añadir Línea</Button></Box>
              <TableContainer component={Paper} variant="outlined">
                <Table size="small">
                  <TableHead sx={{ bgcolor: "#f5f5f5" }}>
                    <TableRow>
                      <TableCell sx={{ width: "50px" }}><b>Inv</b></TableCell>
                      <TableCell sx={{ minWidth: "250px" }}><b>Buscar Servicio (Cód - Desc)</b></TableCell>
                      <TableCell sx={{ minWidth: "200px" }}><b>Descripción Editable</b></TableCell>
                      <TableCell align="right" sx={{ width: "80px" }}><b>Cant.</b></TableCell>
                      <TableCell align="right" sx={{ width: "100px" }}><b>Valor Uni.</b></TableCell>
                      <TableCell align="right" sx={{ width: "100px" }}><b>Subtotal</b></TableCell>
                      <TableCell align="center" sx={{ width: "50px" }}><b>X</b></TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                      ...
                      <TableRow key={idx}>
                        ...
                        <TableCell padding="none" sx={{ px: 1 }}>
                          <Autocomplete
                            ...
                            renderInput={(p) => <TextField {...p} variant="standard" placeholder="Escriba para buscar..." />}
                          />
                        </TableCell>
                        <TableCell padding="none" sx={{ px: 1 }}>
                          <TextField variant="standard" fullWidth value={row.artdescri} onChange={(e) => handleServicioGridChange(idx, "artdescri", e.target.value)} />
                        </TableCell>
                        <TableCell padding="none" sx={{ px: 1 }}>
                          <TextField type="number" variant="standard" inputProps={{ style: { textAlign: "right" } }} value={row.concantidad} onChange={(e) => handleServicioGridChange(idx, "concantidad", e.target.value)} />
                        </TableCell>
                        <TableCell padding="none" sx={{ px: 1 }}>
                          <TextField type="number" variant="standard" inputProps={{ style: { textAlign: "right" }, step: "0.01" }} value={row.convalor} onChange={(e) => handleServicioGridChange(idx, "convalor", e.target.value)} />
                        </TableCell>
                        <TableCell padding="none" sx={{ px: 1 }}>
                          <Typography align="right" variant="body2">${row.contotal}</Typography>
                        </TableCell>
                        <TableCell align="center" padding="none">
                          <IconButton color="error" size="small" onClick={() => { const n = [...servicios]; n.splice(idx, 1); setServicios(n); }}><DeleteIcon /></IconButton>
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
          ...
          <Accordion defaultExpanded sx={accordionStyles}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}><DateRangeIcon color="primary" fontSize="small" /><Typography variant="subtitle2" fontWeight="bold" color="primary">PROYECCIÓN DE FACTURACIÓN</Typography></Box>
            </AccordionSummary>
            <AccordionDetails>
              <Box sx={{ mb: 2, display: "flex", justifyContent: "flex-end" }}><Button variant="contained" color="secondary" startIcon={<AutorenewIcon />} onClick={generarPeriodos}>Generar Períodos</Button></Box>
              <TableContainer component={Paper} variant="outlined">
                <Table size="small">
                  <TableHead sx={{ bgcolor: "#f5f5f5" }}>
                    <TableRow>
                      <TableCell align="center"><b>#</b></TableCell>
                      <TableCell align="center"><b>Mes</b></TableCell>
                      <TableCell align="center"><b>Año</b></TableCell>
                      <TableCell align="center"><b>Estado</b></TableCell>
                      <TableCell align="center"><b>X</b></TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {periodos?.map((per, idx) => (
                      <TableRow key={idx}>
                        ...
                        <TableCell align="center"><IconButton size="small" color="error" onClick={() => handleRemovePeriodo(idx)}><DeleteIcon /></IconButton></TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
