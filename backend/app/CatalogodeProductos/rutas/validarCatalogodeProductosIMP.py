...
# Helper para validar aquí y en el proceso de insertar masivo
def validar_catalogodeproductos(connection, columns: list, required: list, key_columns: list, rows: list):
    ...
    # 2. Validación fila por fila (vacíos y duplicados en el mismo archivo)
    for i, fila in enumerate(rows):
        ...
        # Duplicados en el mismo archivo (Arma una tupla con invcodigo y artcodigo)
        clave = []
    ...
    # 3. Validación contra la Base de Datos (Tabla inmart)
    cols_sql = ", ".join(key_columns)
    cia_val = rows[0]["ciacodigo"]

    # Traemos las claves compuestas que ya existen en esta compañía (invcodigo + artcodigo)
    sql_get_all = text(f"SELECT {cols_sql} FROM inmart WITH (NOLOCK) WHERE ciacodigo = :ciacodigo")
    ...
    for fila in rows:
        ...
        if tuple(clave_fila) in existentes:
            fila["ok"] = False
            fila["feedback"] = "El Artículo ya existe en este Inventario en la base de datos"
...
@bp.route("/validarCatalogodeProductosIMP", methods=["POST"])
@jwt_required()
@api_endpoint
def validarCatalogodeProductosIMP():
    claims = get_jwt()
    ...
    key_columns = data.get("key_columns")  # Desde el front enviarán ["invcodigo", "artcodigo"]
    rows_csv = data.get("rows")

    db.session = get_session(clicianonBD)
    engine = db.session.bind

    # Inyectamos la compañía a cada fila antes de la validación
