...
def validar_impuestos_retenciones(connection, columns: list, required: list, key_columns: list, rows: list):
    ...
    sql_get_all = text(f"SELECT {cols_sql} FROM cxpbimp WHERE ciacodigo = :ciacodigo")
    ...
    for fila in rows:
        ...
        if tuple(clave_fila) in existentes:
            fila["ok"] = False
            fila["feedback"] = "Impuesto/Retención ya existe"
...
@bp.route("/validarImpuestosRetencionesIMP", methods=["POST"])
@cross_origin()
@jwt_required()
@api_endpoint
def validarImpuestosRetencionesIMP():
    claims = get_jwt()
