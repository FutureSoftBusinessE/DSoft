...
def validar_ciudad(connection, columns: list, required: list, key_columns: list, rows: list):
    ...
    sql_get_all = text(f"SELECT {cols_sql} FROM hotbciu")
    ...
    for fila in rows:
        ...
        if tuple(clave_fila) in existentes:
            fila["ok"] = False
            fila["feedback"] = "Ciudad ya existe"
...
@bp.route("/validarCiudadIMP", methods=["POST"])
@jwt_required()
@api_endpoint
def validarCiudadIMP():
    claims = get_jwt()
