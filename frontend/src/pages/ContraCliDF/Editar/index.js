  ...
  useEffect(() => {
    if (fetchedData && fetchedData.concodcontrato) {
      ...
      if (fetchedData.constatus === "I") {
        setIsEditable(false)
        setLockReason("El contrato ya está INACTIVO.")
      } else if (new Date(fetchedData.confecfin) < new Date(getTodayDate())) {
        setIsEditable(false)
        setLockReason("El contrato ha VENCIDO.")
      } else if (hasFacturas) {
        setIsEditable(false)
        setLockReason("El contrato tiene FACTURAS emitidas. Edición bloqueada.")
      } else {
        setIsEditable(true)
        setLockReason("")
      }
    ...
    <ThemeProvider theme={theme}>
      ...
      <div className="main main-app p-3 p-lg-4">
        ...
        <Box sx={{ mb: 2, display: "flex", gap: 1 }}>
          {actionGrabar && isEditable && (
            <Tooltip title="Grabar Cambios">
              <IconButton
                onClick={() => SaveEdicion({ ...formData, servicios, periodos })}
                disabled={isSaving || isFetching || isLoadingCombos}
        ...
        <div style={{ display: "flex", justifyContent: "center", mb: "30px", fontSize: "25px" }}>
          <b>Visualización y Edición de Contrato</b>
        </div>
        ...
        <Box sx={StyledRoot}>
          {!isEditable && !isFetching && lockReason !== "" && (
            <Alert severity="warning" sx={{ mb: 3 }}>
              <b>Aviso:</b> {lockReason} Solo puede visualizar o imprimir.
            </Alert>
          )}
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} sm={6}>
                  <Autocomplete
                    ...
                    value={
                      listaClientes.find((c) => c.id === formData.clicodigo) ||
                      (formData.clicodigo ? { id: formData.clicodigo, label: formData.clicodigo } : null)
                    }
                ...
                <Grid item xs={12} sm={3}>
                  <Autocomplete
                    ...
                    getOptionLabel={(o) => o.label || ""}
                    value={
                      listaTipos.find((t) => t.id === formData.concodigo) ||
                      (formData.concodigo ? { id: formData.concodigo, label: formData.concodigo } : null)
                    }
                      ...
                      <TableRow key={idx}>
                        <TableCell sx={{ minWidth: "200px" }}>
                          <Autocomplete
                            ...
                            getOptionLabel={(o) => o.label || ""}
                            value={
                              listaArticulos.find((a) => a.id === row.artcodigo) ||
                              (row.artcodigo ? { id: row.artcodigo, label: row.artcodigo } : null)
                            }
                            renderInput={(p) => <TextField {...p} variant="standard" />}
                          />
                        </TableCell>
                        <TableCell>
                          <TextField disabled variant="standard" fullWidth value={row.artdescri} />
                        </TableCell>
                        <TableCell>
                          <TextField
                            disabled
          ...
          <Accordion defaultExpanded sx={accordionStyles}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                <Typography variant="subtitle2" fontWeight="bold" color="primary">
                  PROYECCIÓN DE FACTURACIÓN (Solo Lectura)
                </Typography>
              </Box>
            </AccordionSummary>
            <AccordionDetails>
