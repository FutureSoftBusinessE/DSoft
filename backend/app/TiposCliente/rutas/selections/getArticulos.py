...
@bp.route("/getArticulos", methods=["POST"])
@cross_origin()
@jwt_required()
def getArticulos():
    claims = get_jwt()
    ...
    try:
        with engine.connect() as connection:
            if typeSearch == SEARCH_TYPE_HELPER.ID_SEARCH:
                # Search by code: artcodigo
                artcodigo = data.get("artcodigo", "").strip()
                query = text(
                    """
                    SELECT ciacodigo, artcodigo, artdescri, invcodigo
                    FROM inmart
                    WHERE ciacodigo = :ciacodigo AND artcodigo = :artcodigo
                    """
                )
                row = connection.execute(query, {"ciacodigo": sCodCia, "artcodigo": artcodigo}).mappings().fetchone()

                if row:
                    return jsonify({"data": dict(row)})
                else:
                    raise ValidationError(f"Artículo no encontrado: {artcodigo}")
        ...
        raise ValidationError(f"Error obteniendo artículos: {str(e)}")
