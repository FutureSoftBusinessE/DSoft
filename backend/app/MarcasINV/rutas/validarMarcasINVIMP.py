...
def validar_marcasinv(connection, columns: list, required: list, key_columns: list, rows: list):
    ...
    # Validar que las columnas clave (marcodigo) estén configuradas correctamente
    ...
    for i, fila in enumerate(rows):
        ...
        # Verificar campos obligatorios vacíos (marcodigo, mardescri)
        faltantes = []
        ...
        # Detectar duplicados dentro del mismo archivo basándose en marcodigo
        clave = []
        ...
        if clave in vistos:
            fila["ok"] = False
            fila["feedback"] = f"Registro duplicado en el archivo (Marca {clave[0]})"
            continue
    ...
    # Marcar registros que ya existen en la tabla inbmar
    for fila in rows:
        if not fila["ok"]:
            continue
        ...
        if tuple(clave_fila) in existentes:
            fila["ok"] = False
            fila["feedback"] = "Esta Marca ya existe en la base de datos"
...
@bp.route("/validarMarcasINVIMP", methods=["POST"])
@cross_origin()
@jwt_required()
@api_endpoint
def validarMarcasINVIMP():
