...
class SidebarMenu extends Component {
    ...
    for (const option of menu) {
        ...
        navsGroup.push(
          <div className="nav-group" key={option.item_number}>
            <NavLink to={option.link} className="nav-sub-link">
              <div className="nav-label">{icon}{option.label}</div>
            </NavLink>
          </div>
        )
      }
        ...
        const subNavLabels = option.submenu.map((sub) => {
          ...
          return (
            <li className="nav-item" key={sub.item_number}>
              <NavLink to={sub.link} className="nav-sub-link"><i>{subIcon}</i><span>{sub.label}</span></NavLink>
            </li>
          )
        })
        navsGroup.push(
      ...
      if (option.totalNiveles === 4) {
        ...
        const subNavLabels = option.submenu.map((sub) => {
          ...
          return (
            <li className="nav-item" key={sub.item_number}>
              <NavLink to={`Submenu/${sub.label}/${sub.item_number}`} className="nav-sub-link"><i>{subIcon}</i><span>{sub.label}</span></NavLink>
            </li>
          )
        })
        navsGroup.push(
  ...
  populateMenuTree = (menuArr = [], depth = 0) => {
    return menuArr.map((option) => {
      ...
      if (hasSubmenu) {
        ...
        return (
          <details key={option.item_number} style={{ marginBottom: "5px", paddingLeft: paddingLeft, width: "100%" }}>
            <summary className="nav-label" style={{ cursor: "pointer", padding: "8px", fontWeight: "600", display: "flex", alignItems: "center", gap: "10px", listStyle: "none" }}>
        ...
        return (
          <div key={option.item_number} style={{ padding: "6px 8px", paddingLeft: paddingLeft }}>
            <NavLink to={linkTo} className="nav-sub-link" style={{ display: "flex", alignItems: "center", gap: "10px", textDecoration: "none" }}>
              {icon} <span>{option.label}</span>
            </NavLink>
          </div>
        );
  ...
  render() {
    ...
    if (rawMenu) {
      ...
      if (tipoMenu === 1) {
        menuComponent = <div className="tree-menu-container" style={{paddingRight: "10px"}}>{this.populateMenuTree(menuData)}</div>;
      } else {
