...
@bp.route("/getAllBeneficiariosGravamen", methods=["POST"])
@jwt_required()
def getAllTBeneficiariosGravamen():
    claims = get_jwt()
    ...
    with engine.connect() as connection:
        with connection.begin():
            ...
            allowed_columns = [
                {"benegravamen": FILTER_VALUE_TYPE.STRING},
                {"benegrafecisys": FILTER_VALUE_TYPE.DATETIME},
                {"benegrausuisys": FILTER_VALUE_TYPE.STRING},
                {"benegraestisys": FILTER_VALUE_TYPE.STRING},
                {"benegrafecmsys": FILTER_VALUE_TYPE.DATETIME},
                {"benegrausumsys": FILTER_VALUE_TYPE.STRING},
                {"benegraestmsys": FILTER_VALUE_TYPE.STRING},
            ]
            ...
            all_bene_result = [{**{key: value for key, value in dict(row).items() if key != "total"}} for row in result]

    return (
        jsonify(
            {
                "data": all_bene_result,
                "total": total_records,
                "page": page,
                "per_page": per_page,
