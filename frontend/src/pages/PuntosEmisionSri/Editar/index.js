...
import { 
  ...
  MenuItem, Accordion, AccordionSummary, AccordionDetails, Alert, Autocomplete
} from "@mui/material"
...
const EditarPuntosEmisionSri = () => {
  ...
  // Poblar formulario con los datos que vienen desde la grilla
  useEffect(() => {
    if (state) {
      setFormData({
  ...
  // Carga de Combos desde el Backend para mostrar las etiquetas correctas de Localidad y Autorización
  ...
  const handleSubmit = async (e) => {
    ...
    // Solo enviamos los campos permitidos para la edición según la regla de negocio
    await SaveEdicion({ 
      ...
      <div className="main main-app p-3 p-lg-4">
        ...
        <Box sx={StyledRoot} component="form" onSubmit={handleSubmit}>
          ...
          <Alert severity="info" sx={{ mb: 3 }}>
            <b>Modo de Edición:</b> Por seguridad e integridad referencial con los comprobantes SRI, sólo se permite modificar la Descripción y el Estado de la caja.
          </Alert>
          ...
          <Accordion defaultExpanded sx={accordionStyles}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                <Typography variant="subtitle2" fontWeight="bold" color="primary">DATOS DE LA CAJA</Typography>
              </Box>
            </AccordionSummary>
            <AccordionDetails>
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} sm={12}>
                  <Autocomplete
                    ...
                    value={listaLocalidades.find((l) => l.id === formData.loccodigo) || (formData.loccodigo ? { id: formData.loccodigo, label: formData.loccodigo } : null)}
                    renderInput={(p) => <TextField {...p} label="Localidad" InputLabelProps={{ shrink: true }} sx={{ bgcolor: "#f0f0f0" }} />}
                  />
                </Grid>
              </Grid>
            </AccordionDetails>
          ...
          <Accordion defaultExpanded sx={accordionStyles}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                <Typography variant="subtitle2" fontWeight="bold" color="primary">PARÁMETROS DEL SRI (Solo Lectura)</Typography>
              </Box>
            </AccordionSummary>
            <AccordionDetails>
              <Grid container spacing={3}>
                <Grid item xs={12}>
                  <Autocomplete
                    ...
                    getOptionLabel={(o) => o.label || ""}
                    // Buscamos la coincidencia exacta de la autorización por su número o mostramos el número en duro si no cargan los combos a tiempo
                    value={listaAutorizaciones.find((a) => a.sriautnumero === formData.sriautnumero) || (formData.sriautnumero ? { sriautnumero: formData.sriautnumero, label: formData.sriautnumero } : null)}
                    renderInput={(p) => <TextField {...p} label="Nº de Autorización Asignada" InputLabelProps={{ shrink: true }} sx={{ bgcolor: "#f0f0f0" }} />}
                  />
                </Grid>
                <Grid item xs={12} sm={6}>
                  <TextField 
                    disabled
                    fullWidth label="Establecimiento" value={formData.sriserie01} 
                    sx={{ bgcolor: "#f0f0f0" }}
                  />
                </Grid>
                <Grid item xs={12} sm={6}>
                  <TextField 
                    disabled
                    fullWidth label="Punto de Emisión" value={formData.sriserie02} 
                    sx={{ bgcolor: "#f0f0f0" }}
                  />
                </Grid>
              </Grid>
