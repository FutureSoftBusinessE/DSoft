...
// =================================================================
// FUNCIÓN CENTRALIZADA DE EXTRACCIÓN DEFENSIVA (REACT QUERY)
// =================================================================
...
// =================================================================
// COMPONENTE MODAL: BUSCADOR DE LÍNEAS
// =================================================================
...
const EditarCatalogodeProductos = () => {
  const navigate = useNavigate()
  const location = useLocation()
  const queryClient = useQueryClient()
  const { selectedMenuInfo } = useContext(GlobalContext)

  // Extraemos los parámetros de edición
  ...
  // ====================================================================================
  // CARGA DE DATOS DEL PRODUCTO EXISTENTE (EXTRACCIÓN RIGUROSA)
  // ====================================================================================
  const { data: dataEdicion, isLoading: loadEdit } = useQuery({
    queryKey: ["getProductoEdit", invParam, artParam],
    queryFn: async () => {
      if (!invParam || !artParam) return null
      const res = await api.post("/CatalogodeProductos/getProductoId", { invcodigo: invParam, artcodigo: artParam })

      // Extracción rigurosa para manejar la respuesta del interceptor [ { data: {...} }, 200 ]
      let responseData = res?.data?.data || res?.data || null

      if (Array.isArray(responseData)) {
        responseData = responseData[0] // Entramos al diccionario principal de la tupla
      }

      if (responseData && responseData.data) {
        responseData = responseData.data // Extraemos la llave "data" que contiene cabecera, proveedores, etc.
      }
  ...
  useEffect(() => {
    if (dataEdicion && dataEdicion.cabecera) {
      setFormData((prev) => ({ ...prev, ...dataEdicion.cabecera }))
      setProveedoresProd(dataEdicion.proveedores || [])
      setBarrasProd(dataEdicion.barras || [])
      setSustitutos(dataEdicion.sustitutos || [])
      setPrincipiosProd(dataEdicion.principios || [])
      setImagenes(dataEdicion.imagenes || [])
      setDocumentos(dataEdicion.documentos_pdf || [])
    }
  }, [dataEdicion])

  // ====================================================================================
  // QUERIES PARA LOS CATÁLOGOS (COMBOS)
  // ====================================================================================
  ...
  // ====================================================================================
  // MUTACIÓN PARA GRABAR (ACTUALIZAR)
  // ====================================================================================
  const { mutateAsync: SaveEdicionProducto, isPending } = useMutation({
    queryKey: ["isUpdatingProducto"],
    fn: async (payload) => {
      const response = await api.post("/CatalogodeProductos/updateCatalogodeProductos", payload)
      return response.data
    },
    showError: "modal",
    showSuccess: "toast",
    onSuccess: () => {
      // 1. Invalida el caché de la consulta de edición para que traiga datos frescos la próxima vez
      queryClient.invalidateQueries({ queryKey: ["getProductoEdit"] })
  ...
  const handleGuardar = async () => {
    ...
    const payload = {
      ...
      imagenes: imagenes.map((img) => img.base64 || img),
      documentos_pdf: documentos.map((doc) => doc.base64 || doc),

      invcodigoOld: invParam,
      artcodigoOld: artParam,
    }
    await SaveEdicionProducto(payload)
  }
  ...
  return (
    <ThemeProvider theme={theme}>
      <Header />
      <CustomBackdrop isLoading={isPending || loadEdit} />
      ...
      <div className="main main-app p-2 p-lg-3" style={{ backgroundColor: "#f4f6f8", minHeight: "100vh" }}>
        ...
        >
          ...
          >
            {grabarAction ? grabarAction.acccaption : "GRABAR MODIFICACIÓN"}
          </Button>
        </Box>
              ...
              <Grid container spacing={2} pt={1}>
                ...
                <Grid item xs={12} md={3}>
                  <TextField
                    ...
                    size="small"
                    InputProps={{
                      readOnly: true, // BLOQUEADO EN EDICIÓN
...
export default EditarCatalogodeProductos
