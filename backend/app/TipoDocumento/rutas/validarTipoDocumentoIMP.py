...
def validar_tipodoc(connection, columns: list, required: list, key_columns: list, rows: list):
    ...
    sql_get_all = text(f"SELECT {cols_sql} FROM gdocbtipodoc WHERE ciacodigo = :ciacodigo")
    ...
    for fila in rows:
        ...
        if tuple(clave_fila) in existentes:
            fila["ok"] = False
            fila["feedback"] = "Tipo de documento ya existe"
...
@bp.route("/validarTipoDocumentoIMP", methods=["POST"])
@jwt_required()
@api_endpoint
def validarTipoDocumentoIMP():
    claims = get_jwt()
