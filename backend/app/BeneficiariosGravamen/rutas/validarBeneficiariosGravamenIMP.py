...
def validar_beneficiariogravamen(connection, columns: list, required: list, key_columns: list, rows: list):
    ...
    # Indagando a ciacodigo (diccionario) en la primera fila
    cia_val = rows[0]["ciacodigo"]
    sql_get_all = text(f"SELECT {cols_sql} FROM PredbGravBene WHERE ciacodigo = :ciacodigo")
    ...
    for fila in rows:
        ...
        if tuple(clave_fila) in existentes:
            fila["ok"] = False
            fila["feedback"] = "Beneficiario de gravamen ya existe"
...
@bp.route("/validarBeneficiariosGravamenIMP", methods=["POST"])
@cross_origin()
@jwt_required()
@api_endpoint
def validarBeneficiariosGravamenIMP():
    claims = get_jwt()
