...
const CrearLineasINV = () => {
      ...
      try {
        ...
        if (resultData) {
          ...
          setConfigCia({
            ...
            totalLength: totalLength, maxLevels: resultData.cianiveleslin || lengths.length
          })
        }
      } catch (error) {
  ...
  const { mutateAsync: SaveCreacionLinea, isPending: isSaving, isError } = useMutation({
  ...
  const handleBlurCodigo = async () => {
    let code = formData.lincodigo.trim()
    if (!code) {
    ...
    let segments = []
    let currentIndex = 0
    for (let len of configCia.lengths) {
      segments.push(clean.substring(currentIndex, currentIndex + len))
      currentIndex += len
    }

    let formattedCode = configCia.delimiter ? segments.join(configCia.delimiter) : clean
    let nivelCalc = 1
    let parentCodeBD = ""
          ...
          <Paper elevation={3} sx={{ p: 4, borderRadius: 3, background: "white" }} component="form" onSubmit={handleSubmit}>
            ...
            <Grid container spacing={3} alignItems="center">
              <Grid item xs={12} sm={4}>
                <TextField fullWidth label="Código *" value={formData.lincodigo} onChange={(e) => handleInputChange("lincodigo", e.target.value)} onBlur={handleBlurCodigo} error={isError && !formData.lincodigo} inputProps={{ maxLength: 20 }} InputLabelProps={{ shrink: true }} />
              </Grid>
              ...
              <Grid item xs={12} sm={12}>
                <TextField fullWidth label="Descripción *" value={formData.lindescri} onChange={(e) => handleInputChange("lindescri", e.target.value)} error={isError && !formData.lindescri} inputProps={{ maxLength: 40 }} InputLabelProps={{ shrink: true }} />
              </Grid>
              <Grid item xs={12} sm={4}>
              ...
              <Grid item xs={12} sm={8}>
                <TextField fullWidth value={isLoadingPadre ? "Buscando..." : padreInfo.descri} disabled variant="filled" sx={{ backgroundColor: "#f9fafb", '& .MuiInputBase-input.Mui-disabled': { WebkitTextFillColor: padreInfo.descri.includes("NO ENCONTRADO") ? "#d32f2f" : "#8e24aa", fontWeight: 'bold' } }} />
              </Grid>
