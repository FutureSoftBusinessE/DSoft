...
@bp.route("/getMarcas", methods=["POST"])
@cross_origin()
@jwt_required()
def getMarcas():
    claims = get_jwt()
    ...
    try:
        with engine.connect() as connection:
            if typeSearch == SEARCH_TYPE_HELPER.ID_SEARCH:
                # Search by code: marcodigo
                marcodigo = data.get("marcodigo", "").strip()
                query = text(
                    """
                    SELECT ciacodigo, marcodigo, mardescri
                    FROM inbmar
                    WHERE ciacodigo = :ciacodigo AND marcodigo = :marcodigo
                    """
                )
                row = connection.execute(query, {"ciacodigo": sCodCia, "marcodigo": marcodigo}).mappings().fetchone()

                if row:
                    return jsonify({"data": dict(row)})
                else:
                    raise ValidationError(f"Marca no encontrada: {marcodigo}")
        ...
        raise ValidationError(f"Error obteniendo marcas: {str(e)}")
