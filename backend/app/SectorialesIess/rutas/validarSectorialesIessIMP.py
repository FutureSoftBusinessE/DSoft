...
def validar_sectorialesiess(connection, columns: list, required: list, key_columns: list, rows: list):
    ...
    # Validar que las columnas clave estén en los requeridos y en el total de columnas
    ...
    # 2. Validación fila por fila (Campos vacíos y duplicados internos)
    for i, fila in enumerate(rows):
        ...
        # Detectar duplicados dentro del mismo archivo (PK Compuesta: seccodigo + secanio)
        clave = []
        ...
        if clave in vistos:
            fila["ok"] = False
            fila["feedback"] = "Registro duplicado en el archivo (Código IESS y Año)"
            continue

        vistos.add(clave)

    # 3. Verificación contra Base de Datos (Multitenancy)
    cols_sql = ", ".join(key_columns)
    cia_val = rows[0]["ciacodigo"]
    # Consultamos registros existentes para la compañía actual
    sql_get_all = text(f"SELECT {cols_sql} FROM nomsectorialiess WHERE ciacodigo = :ciacodigo")
    ...
    # Marcar registros que ya existen en la base de datos
    for fila in rows:
        if not fila["ok"]:
            continue
        ...
        if tuple(clave_fila) in existentes:
            fila["ok"] = False
            fila["feedback"] = "El Sectorial ya existe en la base de datos para este código y año"
...
@bp.route("/validarSectorialesIessIMP", methods=["POST"])
@cross_origin()
@jwt_required()
@api_endpoint
def validarSectorialesIessIMP():
