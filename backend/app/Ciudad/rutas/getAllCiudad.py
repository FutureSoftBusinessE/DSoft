...
@bp.route("/getAllCiudad", methods=["POST"])
@cross_origin()
@jwt_required()
def getAllCiudad():
    claims = get_jwt()
    ...
    with engine.connect() as connection:
        with connection.begin():
            ...
            allowed_columns = [
                {"ciucodigo": FILTER_VALUE_TYPE.STRING},
                {"ciudescri": FILTER_VALUE_TYPE.STRING},
                {"ciustatus": FILTER_VALUE_TYPE.STRING},
                {"ciufecsys": FILTER_VALUE_TYPE.DATETIME},
                {"ciuhorsys": FILTER_VALUE_TYPE.DATETIME},
                {"ciuususys": FILTER_VALUE_TYPE.STRING},
                {"ciudinardap": FILTER_VALUE_TYPE.STRING},
            ]
            ...
            all_ciudad_result = [{**{key: value for key, value in dict(row).items() if key != "total"}} for row in result]

    return (
        jsonify(
            {
                "data": all_ciudad_result,
                "total": total_records,
                "page": page,
                "per_page": per_page,
