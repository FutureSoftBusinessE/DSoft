...
// Mapeo exacto igual que en Nota de Débito
const ACCIONES = {
  CREAR: "CREAR",
  BUSCAR: "BUSCAR",
  EDITAR: "",
...
const NotaCreditoDF = () => {
  ...
  const handleAutorizar = async (row) => {
    const { nccodigo } = row.original

    Swal.fire({
      title: "¿Autorizar Nota de Crédito?",
      text: `¿Está seguro que desea enviar el documento ${nccodigo} al SRI?`,
      icon: "warning",
      ...
      if (result.isConfirmed) {
        setIsLoading(true)
        try {
          const response = await api.post("/NotaCreditoDF/autorizarSRI", { nccodigo })
          if (response.data.success) {
            Swal.fire("¡Autorizado!", `El documento ${nccodigo} fue autorizado con éxito.`, "success")
            queryClient.invalidateQueries(["listarNotasCreditoDF"])
          } else {
  ...
  const generateRIDE = async (row) => {
    ...
    try {
      // El backend extrae 'facnumfac' de la tabla genérica, enviamos el 'nccodigo' mapeado a esa variable
      const response = await api.post("/NotaCreditoDF/descargarRIDE", { facnumfac: nccodigo })
      if (response.data.success && response.data.ridePDF) {
        const linkSource = `data:application/pdf;base64,${response.data.ridePDF}`
        const downloadLink = document.createElement("a")
        const fileName = `NotaCredito_${response.data.claveAcceso || nccodigo}.pdf`
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <div className="main main-app p-3 p-lg-4">
        ...
        >
          <b>Gestión de Notas de Crédito Electrónicas</b>
        </div>
        <Box className={StyledRoot}>
          <CustomConditionalActionsTableServer
            endpoint="/NotaCreditoDF/listar"
            errorMsgFilterSearch="Error en cargar datos"
            queryKeyModal="listarNotasCreditoDF"
            perPage={10}
            rowActionsWidthTable={200}
            idSearchField="nccodigo" // Identificador único de fila para Nota de Crédito
            // ==========================================
            ...
            rowActions={(row) => {
              ...
              if (autorizarAction && !isAuthorized && row.original.nccodigo && row.original.ncstatus === "A") {
                actions.push({
              ...
              if (rideAction && isAuthorized && row.original.nccodigo) {
                actions.push({
                ...
                toolbarActions.push({
                  ...
                  actions: [
                      ...
                      onClick: ({ columns, data }) => {
                        if (device === "sm") {
                          return handleExportDataPdfSMScreen(
                            ...
                            `Notas de Crédito ${new Date().toLocaleString()}`,
                          )
                        }
                        handleExportDataPdfLGScreen(
                          columns,
                    ...
                    },
                    {
                      ...
                      icon: getIconComponent(exportarAction?.accnameicono, exportarAction?.acctipoico),
                      onClick: ({ data }) => {
                        handleAllExportDataCSV(data, `Notas de Crédito ${new Date().toLocaleString()}`)
                      },
                    },
                  ],
                })
            ...
            columnsTable={[
              {
                accessorKey: "nccodigo",
                header: "Nº Nota de Crédito",
                size: 150,
