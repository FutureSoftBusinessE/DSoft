...
# Helper para validar aquí y en el proceso de insertar[cite: 18]
def validar_tipodecompania(connection, columns: list, required: list, key_columns: list, rows: list):

    # 1. Validaciones estructurales básicas[cite: 18]
    ...
    # 2. Validación fila por fila (vacíos y duplicados en el mismo archivo)[cite: 18]
    for i, fila in enumerate(rows):
        ...
        # Campos required vacíos[cite: 18]
        faltantes = []
        ...
        # Duplicados en el mismo archivo[cite: 18]
        clave = []
    ...
    # 3. Validación contra la Base de Datos (Tabla siactipocompania)[cite: 18]
    cols_sql = ", ".join(key_columns)

    # Traemos las claves que ya existen en la base de datos (Global, sin ciacodigo)[cite: 18]
    sql_get_all = text(f"SELECT {cols_sql} FROM siactipocompania")
    ...
    for fila in rows:
        ...
        if tuple(clave_fila) in existentes:
            fila["ok"] = False
            fila["feedback"] = "El Tipo de Compañía ya existe en la base de datos"

    # 4. Cálculo del resumen[cite: 18]
...
@bp.route("/validarTipoDeCompaniaIMP", methods=["POST"])
@jwt_required()
@api_endpoint
def validarTipoDeCompaniaIMP():
    claims = get_jwt()
    ...
    # No inyectamos ciacodigo porque la tabla se rige de forma global[cite: 18]
