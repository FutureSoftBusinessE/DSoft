...
def validar_parroquia(connection, columns: list, required: list, key_columns: list, rows: list):
    ...
    sql_get_all = text(f"SELECT {cols_sql} FROM cxcbparroquia")
    ...
    for fila in rows:
        ...
        if tuple(clave_fila) in existentes:
            fila["ok"] = False
            fila["feedback"] = "Parroquia ya existe"
...
@bp.route("/validarParroquiaIMP", methods=["POST"])
@jwt_required()
@api_endpoint
def validarParroquiaIMP():
    claims = get_jwt()
