...
const EditarTransportistaDF = () => {
  ...
  const validarIdentificacionTransportista = (tipoTrans, numero) => {
    ...
    if (digitos[2] < 6) {
      ...
      if ((suma % 10 === 0 ? 0 : 10 - (suma % 10)) !== digitos[9])
        return { ok: false, msg: "Dígito verificador de Cédula/RUC incorrecto" }
    }
    return { ok: true }
  }
  ...
  const handleInputChange = (field, value, isEmail = false) => {
    const val = typeof value === "string" && !isEmail && field !== "transstatus" ? value.toUpperCase() : value
          ...
          <Paper elevation={3} sx={{ p: 4, borderRadius: 3 }} component="form" onSubmit={handleSubmit}>
            ...
            <Grid container spacing={3} justifyContent="center">
              <Grid item xs={12} sm={6}>
                <TextField
                  fullWidth
                  label="Placa Sugerida para la Guía de Remisión Electrónica"
