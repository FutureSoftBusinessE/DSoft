    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if sri_data_list and len(sri_data_list) > 0:
                ...
                # SOLUCIÓN: formatted_data fue des-indentado para que siempre se declare
                formatted_data = {
                ...
                try:
                    ...
                    with engine.connect() as conn:
                        query = text("SELECT procodigo FROM cxpmprov WHERE proruc = :ruc")
                    ...
                    # Si la BD local falla, retornamos False en vez de romper la respuesta del SRI
                    formatted_data["ya_registrado"] = False
                    print(f"Error en verificación SIAC: {e}")

                return formatted_data
            else:
                raise APIError(f"RUC {ruc_clean} no encontrado.")
        else:
            raise APIError("El SRI no responde adecuadamente o el RUC no existe.")
    except Exception as e:
