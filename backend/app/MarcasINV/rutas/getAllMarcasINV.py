...
@bp.route("/getAllMarcasINV", methods=["POST"])
@cross_origin()
@jwt_required()
def getAllMarcasINV():
    ...
    with engine.connect() as connection:
        with connection.begin():
            # 3. Definir columnas permitidas para filtros dinámicos según estructura inbmar
            allowed_columns = [
                {"marcodigo": FILTER_VALUE_TYPE.STRING},
                {"mardescri": FILTER_VALUE_TYPE.STRING},
                {"marstatus": FILTER_VALUE_TYPE.STRING},
                {"marfecisys": FILTER_VALUE_TYPE.DATETIME},
                {"marhorisys": FILTER_VALUE_TYPE.DATETIME},
                {"marusuisys": FILTER_VALUE_TYPE.STRING},
                {"marfecmsys": FILTER_VALUE_TYPE.DATETIME},
                {"marhormsys": FILTER_VALUE_TYPE.DATETIME},
                {"marusumsys": FILTER_VALUE_TYPE.STRING},
            ]
            ...
            final_query, params = build_paginated_query(
                ...
                order_by=[
                    "mardescri ASC",  # Ordenamiento por defecto: Alfabético
                ],
                filters=filters,
                page=page,
                per_page=per_page,
            ...
            all_marcas_result = [{**{key: value for key, value in dict(row).items() if key != "total"}} for row in result]

    # 8. Retorno estructurado para el componente de grilla en React
    return (
        jsonify(
            {
                "data": all_marcas_result,
                "total": total_records,
                "page": page,
                "per_page": per_page,
