...
# Helper para validar Servicios de Notas de Débito y Crédito
def validar_servicios_ndnc(connection, columns: list, required: list, key_columns: list, rows: list):
    ...
    for i, fila in enumerate(rows):
        ...
        # 2. Regla de Negocio: Validar Tipo de Servicio (Débito/Crédito)
        tipo_servicio = fila.get("serncnd")
        if tipo_servicio and isinstance(tipo_servicio, str):
            tipo_servicio = tipo_servicio.strip().upper()
            fila["serncnd"] = tipo_servicio  # Normalizamos a mayúscula
            if tipo_servicio not in tipos_validos:
                errores_fila.append(f"Tipo de servicio inválido ({tipo_servicio}). Use: {', '.join(tipos_validos)} (D=Débito, C=Crédito).")
        ...
        if clave in vistos:
            errores_fila.append("El código de servicio está duplicado dentro de este mismo archivo.")
        else:
            vistos.add(clave)
    ...
    sql_get_all = text(f"SELECT {cols_sql} FROM cxcbser WHERE ciacodigo = :ciacodigo")
    ...
    for fila in rows:
        ...
        if tuple(clave_fila) in existentes:
            fila["ok"] = False
            fila["feedback"] = "Este código de servicio ya existe en el sistema."
...
@bp.route("/validarServiciosNDNCIMP", methods=["POST"])
@jwt_required()
@api_endpoint
def validarServiciosNDNCIMP():
    claims = get_jwt()
