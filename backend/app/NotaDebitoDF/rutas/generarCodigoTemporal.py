...
def generarCodigoTemporal(cjacodigo):
    """Genera el código NEMOTÉCNICO de la ND según la caja seleccionada"""
    claims = get_jwt()
    ...
    with engine.connect() as connection:
        ...
        if not serie:
            raise ValidationError("No existe configuración SRI (05) para la caja seleccionada.")

        srisecact = int(serie["srisecact"] or 0)
        secuencia_actual = srisecact + 1
        # 2 dígitos para cumplir varchar(18)
        ...
        facnumfac = f"D{year}{sriserie01}{sriserie02}{secuencia_actual:09}"

    return {"data": facnumfac}
