...
def descargar_ride():
    try:
        ...
        # Nota: Buscamos por guinumero, no por facnumfac
        ...
        with engine.connect() as conn:
            res = conn.execute(text("SELECT sripdf, sriclave FROM siacdocelectronicos WHERE ciacodigo=:cia AND facnumfac=:gui"), {"cia": ciacodigo, "gui": guinumero}).fetchone()

            if not res or not res[0]:
                return jsonify({"success": False, "message": "No se encontró el PDF (RIDE) en la base de datos."}), 404
