...
const CrearImpuestoRetencion = () => {
  const navigate = useNavigate()
  const { selectedMenuInfo } = useContext(GlobalContext)

  const {
    mutateAsync: SaveCreacionImpuestoRetencion,
    isPending: isSavingCreacionImpuestoRetencion,
    isError: isErrorSavingCreacion,
  } = useMutation({
    queryKey: ["isCreatingImpuestoRetencion"],
    fn: async (data) => {
      const response = await api.post("/ImpuestosRetenciones/crearImpuestosRetenciones", data)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!dataToSave.impid) {
        showWarning("Ingresa el código")
        return
      }
      if (!dataToSave.impdescri) {
        showWarning("Ingresa la descripción del impuesto/retención")
        return
      }
      if (isNaN(dataToSave.impporcent) || dataToSave.impporcent < 0) {
        showWarning("Porcentaje que Aplica debe ser un número mayor o igual a 0")
        return
      }

      await SaveCreacionImpuestoRetencion(dataToSave)
    } catch (error) {
    ...
    <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={esLocale}>
      <ThemeProvider theme={theme}>
        ...
        <div className="main main-app p-3 p-lg-4">
            ...
            {toolbarActions.map((action) => (
              <Tooltip title={action.label} key={action.key}>
                <IconButton
                  ...
                  disabled={isSavingCreacionImpuestoRetencion}
                  sx={{
                    border: "1px solid",
                    borderColor: "divider",
                    borderRadius: 1,
          ...
          >
            <b>Crear Impuesto/Retención</b>
          </div>

          <CustomBackdrop isLoading={isSavingCreacionImpuestoRetencion} />
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={8} md={6}>
                  <TextField
                    ...
                    onChange={(e) => handleInputChange("impid", e.target.value.toUpperCase())}
                    error={isErrorSavingCreacion}
                    inputProps={{ maxLength: MAX_LENGTHS.impid }}
                    helperText="Ingresa el código manualmente"
                  />
                </Grid>
...
export default CrearImpuestoRetencion
