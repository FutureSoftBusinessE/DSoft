...
def validar_provincia(connection, columns: list, required: list, key_columns: list, rows: list):
    ...
    sql_get_all = text(f"SELECT {cols_sql} FROM rhbprov")
    ...
    for fila in rows:
        ...
        if tuple(clave_fila) in existentes:
            fila["ok"] = False
            fila["feedback"] = "Provincia ya existe"
...
@bp.route("/validarProvinciaIMP", methods=["POST"])
@cross_origin()
@jwt_required()
@api_endpoint
def validarProvinciaIMP():
    claims = get_jwt()
