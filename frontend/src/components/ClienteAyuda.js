...
const ClienteAyuda = () => {
  ...
  function useGetClientes(pageNumber, filters) {
    return useQuery({
      queryKey: ["solicitudDeIngresoCliente", pageNumber, filters],
      queryFn: async () => {
        const response = await fetchwrapper(`/solicitudDeIngreso/getClientes`, {
          method: "POST",
          headers: {
  ...
  // Borra todos los filtros y recarga los datos
  const handleClearFilters = () => {
    setFilters({})
    setPage(1) // Opcional: Regresa a la primera página al borrar los filtros
  }
...
const CustomTable = ({ data, isLoading, isError, totalPages, currentPage, onPageChange, onFilterChange }) => {
  ...
  const table = useMaterialReactTable({
    columns,
    data,
    initialState: { columnVisibility: { ciacodigo: false } },
    enableEditing: false,
    enableSorting: false,
    ...
    muiToolbarAlertBannerProps: isError ? { color: "error", children: "Error en cargar los clientes" } : undefined,
  ...
  useEffect(() => {
    if (table.getSelectedRowModel().rows[0] != null) {
      const codCliente = getSelectedRowData(table.getSelectedRowModel().rows)[0].clicodigo
      const clienteDescri = getSelectedRowData(table.getSelectedRowModel().rows)[0].clinombre
      setClienteSeleccionado(codCliente)
      setClienteSeleccionadoDescri(clienteDescri)
    }
  }, [table.getSelectedRowModel(), setClienteSeleccionado, setClienteSeleccionadoDescri])
