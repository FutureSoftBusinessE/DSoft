...
const Provincia = () => {
  ...
  const { mutateAsync: SaveEliminacionProvincia, isPending: isDeletingProvincia } = useMutation({
    queryKey: ["isDeletingProvincia"],
    fn: async (data) => {
      const response = await api.post("/Provincia/eliminarProvincia", data)
      return response.data
    },
    showError: "modal",
    showSuccess: "toast",
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: ["Provincia"] })
    },
  })
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <div className="main main-app p-3 p-lg-4">
        ...
        <CustomBackdrop isLoading={isDeletingProvincia} />

        <Box className={StyledRoot}>
          <ModalImportCSV
            open={openModal}
            onClose={() => setOpenModal(false)}
            templateFileName="PlantillaCreacionProvincia.csv"
            fieldConfigs={{
              procodigo: { required: true, key: true },
              prodescri: { required: true },
              prostatus: { required: false },
              profecsys: { required: false },
              prohorsys: { required: false },
              proususys: { required: false },
            }}
            maxFileSize={10 * 1024 * 1024}
            validateEndpoint="/Provincia/validarProvinciaIMP"
            insertEndpoint="/Provincia/insertarProvinciaIMP"
            onImportComplete={() => qc.invalidateQueries({ queryKey: ["Provincia"] })}
          />

          <CustomConditionalActionsTableServer
            endpoint="/Provincia/getAllProvincia"
            errorMsgFilterSearch="Error en cargar datos"
            queryKeyModal="Provincia"
            perPage={10}
            ...
            rowActions={(row) => {
              ...
              const actions = [
                ...
                },
                {
                  ...
                  onClick: async (r) => {
                    try {
                      await SaveEliminacionProvincia(r.original)
                    } catch (err) {
                      console.error("Error eliminando la provincia:", err)
                    }
                  },
                },
              ]
                ...
                },
                {
                  ...
                  actions: [
                      ...
                      onClick: ({ columns, data }) => {
                        if (device === "sm") {
                          return handleExportDataPdfSMScreen(
                            ...
                            `Reporte de Provincias ${new Date().toLocaleString()}`,
                          )
                        }
                        handleExportDataPdfLGScreen(
                          columns,
                    ...
                    },
                    {
                      ...
                      icon: getIconComponent(exportarAction?.accnameicono, exportarAction?.acctipoico),
                      onClick: ({ data }) => {
                        handleAllExportDataCSV(data, `Reporte de Provincias ${new Date().toLocaleString()}`)
                      },
                    },
                  ],
                },
