...
def validar_sector_comercial_cliente(connection, columns: list, required: list, key_columns: list, rows: list):
    ...
    sql_get_all = text(f"SELECT {cols_sql} FROM cxcbacteconomicas")
    ...
    for fila in rows:
        ...
        if tuple(clave_fila) in existentes:
            fila["ok"] = False
            fila["feedback"] = "Sector comercial cliente ya existe"
...
@bp.route("/validarSectorComercialClienteIMP", methods=["POST"])
@jwt_required()
@api_endpoint
def validarSectorComercialClienteIMP():
    claims = get_jwt()
