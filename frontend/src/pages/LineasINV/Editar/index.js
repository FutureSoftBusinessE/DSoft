...
import { Box, Paper, TextField, Tooltip, IconButton, Grid, MenuItem, Typography, InputAdornment } from "@mui/material"
...
const EditarLineasINV = () => {
  ...
  useEffect(() => {
    const initData = async () => {
      try {
        const configRes = await api.get("/LineasINV/getConfigLineas")
        ...
        if (resultCfg) {
          ...
          if (state) {
            setFormData({
              lincodigo: state.lincodigo || "",
              lindescri: state.lindescri || "",
              coscodigo: state.coscodigo || null,
              lintipo: state.lintipo || "T",
  ...
  const { mutateAsync: SaveEdicionLinea, isPending: isSaving } = useMutation({
    queryKey: ["isEditingLineaINV"],
    fn: async (data) => {
  ...
  const handleBlurCodigo = async (overrideCode = null, overrideConfig = null) => {
    ...
    for (let i = segments.length - 1; i >= 0; i--) {
      if (!/^0+$/.test(segments[i])) {
        ...
        if (i > 0) {
          ...
          parentCodeVisual = config.delimiter ? parentSegs.join(config.delimiter) : parentCodeBD
        }
        break
      }
    }
    ...
    if (parentCodeBD) {
      ...
      try {
        ...
        setPadreInfo({ codigoVisual: parentCodeVisual, codigoBD: parentCodeBD, descri: "ERROR" })
      } finally {
...
export default EditarLineasINV